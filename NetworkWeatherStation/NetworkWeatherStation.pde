...
// compile and load onto SparkFun USB Weather Board V3 using Arduino development envrionment, download from www.arduino.cc

// uses the SHT15x library by Jonathan Oxer et.al.
// download from https://github.com/practicalarduino/SHT1x, place in your Arduino sketchbook under "libraries/SHT1x/"

// uses the SFE_BMP085 library by SparkFun with math from http://wmrx00.sourceforge.net/Arduino/BMP085-Calcs.pdf
// place in your Arduino sketchbook under "libraries/SFE_BMP085"
...
// only load into variables if the settings are valid (not -1 == 0xFF)
// note that ints are two bytes, longs are four
{
  int tempint;
  long templong;
